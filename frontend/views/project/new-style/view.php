    ...
    <section id="button-steps">
        <div class="row">
            ...
            <?php foreach ($steps as $key => $step):?>
                ...
                <?php if($step['StepProjectId'] < $progress['avilableStep'] || $progress['avilableStep'] == 4 ): ?>
                    <!-- This behavior is only when the step project is Sample Reception and it hasn't actions-->
                    <!-- SPECIAL CASE: Breeder behaviors -->
                    <?php if(($step['StepProjectId'] == StepProject::MARKERS_SELECTION 
                            && $model->StepProjectId >= StepProject::MARKERS_SELECTION 
                            && count($model->markersByProjects) == 0 ) || 
                            ($step['StepProjectId'] >= StepProject::GRID_DEFINITION 
                            && count($model->samplesByProjects) == 0)): ?>
                        <!-- If stepId of project is >= to SEND, and there are not markers selected.
                        The admin can select markers on selct-markers view-->
                    
                    
                        <!-- This case is for projects that are already on grid_definition on previous verison-->
                        <a href="<?= $step['url']."?id=".$model->ProjectId?>" />
                    <?php else: ?>
                        <a href="#" onclick="return actionStep('<?= $step['url']."?id=".$model->ProjectId?>');" />
                    <?php endif; ?>
                    <!-- End sample recpetion behavior-->
                <?php else: ?>
                                                
                    <?php  if($step['url'] != 'not-allow' && 
                            ($step['StepProjectId'] != StepProject::DNA_EXTRACTION 
                            &&  $step['StepProjectId'] != StepProject::REPORT ) || 
                            (count($model->markersByProjects) > 0 
                            && $model->StepProjectId == StepProject::GRID_DEFINITION) ||
                            ($model->StepProjectId == StepProject::SENT && $progress['avilableStep'] == StepProject::SAMPLE_DISPATCH )):  ?>
                        <!-- This actions must be performed by Ajax -->    
                        <a href="<?= $step['url']?>">
                    <?php else: ?>
                        <a href="#" onclick="return actionStep('<?= $step['url']."?id=".$model->ProjectId?>');" />
                    <?php endif; ?>
