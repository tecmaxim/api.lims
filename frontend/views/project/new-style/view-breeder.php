        ...
        <div class="row">
            <div class="col-md-12">
                <div class="progress">
                    ...
                    <div class="item-step <?= $class;?>" style="left:<?= $left?>%;"><?= $step['StepProjectId'] == common\models\StepProject::REPORT ? '8':$step['StepProjectId'];?></div>
        ...
        <div class="row">
            ...
            <?php foreach ($steps as $key => $step):?>
                ...
                <?php if($step['StepProjectId'] < $progress['avilableStep'] || $progress['avilableStep'] == 4): ?>
                    ...
                    <?php// if($step['StepProjectId'] != common\models\StepProject::SAMPLE_RECEPTION ): ?>
                        <a href="#" onclick="return actionStep('<?= $step['url']."?id=".$model->ProjectId?>');" />
                    <?php //endif; ?>
                    <!-- End sample recpetion behavior-->
                    
                <?php else: ?>
                    <?php if($step['StepProjectId'] != common\models\StepProject::DNA_EXTRACTION &&  $step['StepProjectId'] != common\models\StepProject::REPORT ):  ?>
                        <a href="<?= $step['url']?>">
                    <?php else: ?>
                        <a href="#" onclick="return actionStep('<?= $step['url']."?id=".$model->ProjectId?>');" />
                    <?php endif; ?>
                ...
                <div class="col-lg-3 col-sm-12 col-xs-12 col-md-4">
                    <div class="new-button-steps <?= $class ?>">
                        ...
                        <div class="label-step">
                           <?= $step['Name'] === 'Sent' ? 'Shipment & Plates Status':$step['Name']?>
                        </div>
                    </div>
                </div>
                </a>
